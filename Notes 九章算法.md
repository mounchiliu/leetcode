...
一般，使用DFS的问题类似于**找所有满足某个条件的方案**

我们可以转换成隐式图搜索的问题：找图中所有满足条件的路径

### 2. 时间复杂度

- 组合问题：组合中的元素顺序都是无关的，时间复杂度与 **2^n** 相关

- 排列问题: 组合中的元素顺序都是相关的，时间复杂度与 **n!** 相关

- 通用的DFS时间复杂度计算公式： O(答案个数 * 构造每个答案的时间)，其中答案个数与上述条件相关

**Note:**

- 剪枝：当前节点已经不能符合要求，则没有必要继续递归往下搜索

- 剪枝不会改变时间复杂度，但是正常情况下会极大程度提高程序的运算效率
